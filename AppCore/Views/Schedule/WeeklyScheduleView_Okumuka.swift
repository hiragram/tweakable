...
struct WeeklyScheduleView_Okumuka: View {
    ...
    var body: some View {
        ...
        .overlay(alignment: .top) {
            if !isEditable {
                NetworkStatusBanner_Okumuka(theme: theme)
            }
        }
    }
...
struct DayScheduleRow_Okumuka: View {
    ...
    var body: some View {
        HStack(spacing: theme.spacing.sm) {
            ...
            HStack(spacing: theme.spacing.sm) {
                ScheduleStatusButton_Okumuka(
                    status: entry.dropOffStatus,
                    theme: theme,
                    isEnabled: isEditable,
                    onTap: onDropOffTap
                )
                .accessibilityIdentifier(WeeklyScheduleView_Okumuka.AccessibilityID.dropOffButton(dayIndex: index))
                ...
                .accessibilityIdentifier(WeeklyScheduleView_Okumuka.AccessibilityID.pickUpButton(dayIndex: index))
            }
            .fixedSize(horizontal: true, vertical: false)
        }
        .padding(.horizontal, theme.spacing.md)
...
struct ScheduleStatusButton_Okumuka: View {
    ...
    var body: some View {
        ...
        .buttonStyle(ScheduleButtonStyle_Okumuka())
        .disabled(!isEnabled)
    }
}
...
struct ScheduleButtonStyle_Okumuka: ButtonStyle {
...
#Preview("WeeklyScheduleView_Okumuka") {
    ...
    return WeeklyScheduleView_Okumuka(
...
#Preview("WeeklyScheduleView_Okumuka - Loading") {
...
#Preview("WeeklyScheduleView_Okumuka - Saving") {
    let calendar = PreviewDate.calendar
    let today = PreviewDate.today
    let userID = UUID()
    let groupID = UUID()
...
#Preview("WeeklyScheduleView_Okumuka - Offline") {
    let calendar = PreviewDate.calendar
    let today = PreviewDate.today
    let userID = UUID()
    let groupID = UUID()

...
タスクに合わせたエージェントチームを作成して、様々な視点からタスクを検討してほしい。
エージェントチームには以下の役割を必ず参加させること。

- 要件定義の責任者。シンプルで直感的な要件、細かいエッジケースまで含めた網羅的な考慮が求められる。
- 開発者。より良いソフトウェアとしてのあるべき姿を追求し、中長期的なコードの品質にまでコミットメントを持つ。
- 批判的なステークホルダー。デビルズアドボケート役。各エージェントの方針や成果を批判的な視点で精査し、適宜見直しややり直しを提言する。

他に、タスクにとって必要だとおもうエージェントを追加してよい。
